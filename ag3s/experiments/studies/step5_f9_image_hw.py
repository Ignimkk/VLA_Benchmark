"""F9 판정 — `lift()` 의 `image_hw` 기본값이 실측에서 실제로 어긋나는가.
    ...
    MUJOCO_GL=osmesa PYTHONPATH=/mnt/dev/work /mnt/dev/work/.venv-ag3s/bin/python -m \\
        benchmark.ag3s.experiments.studies.step5_f9_image_hw --records run_0004 \\
...
def main() -> None:
    _style()
    from benchmark.ag3s.stages import attention_lifting as al
    from benchmark.ag3s.runtime import pipeline as pl
