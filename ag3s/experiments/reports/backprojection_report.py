...
def fit_plane(points: np.ndarray) -> Optional[tuple[np.ndarray, float, np.ndarray, int]]:
    ...
    """
    from benchmark.ag3s.stages.support_surface import fit_plane_ransac
...
def main() -> None:
    ...
    ap.add_argument("--out-doc", default="benchmark/ag3s/docs/archive/step-verification-20260904/step-02-backprojection.md")
...
| 역투영 | `benchmark.ag3s.stages.reconstruction.backproject` (AG3S 본체 코드 그대로) |
...
공통 규격은 `benchmark/ag3s/experiments/common/figstyle.py`에 있다.
