...
실행:
    MUJOCO_GL=osmesa PYTHONPATH=/mnt/dev/work .venv-ag3s/bin/python -m \
        benchmark.ag3s.experiments.a4_task_tsdf --records run_0004 --frames 24
"""
...
def _count_in_box(occ, grid, lo, hi):
    """`lo..hi` 상자 안의 점유 복셀 수."""
    from benchmark.ag3s.esdf import OCCUPIED
...
def main() -> None:
    ...
    from benchmark.ag3s.esdf import CameraDepth, EsdfBuilder, TsdfVolume
