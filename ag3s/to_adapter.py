...
def build_constraint_set(
    ...
    *,
    ...
    attached_dropped = int(spec.layout.get("n_attached_dropped", 0))
    if attached_dropped:
        ...
        notes.append(
            ...
            f"{spec.layout.get('max_attached_primitives')} and are NOT constrained"
        )
        validity = ConstraintValidity.worst(validity, ConstraintValidity.INCOMPLETE)

    overflow = int(spec.layout.get("n_overflow_spheres", 0))
    if overflow:
        notes.append(
    ...
    elif not candidates and not support_surfaces:
