...
| attention | `benchmark/ag3s/asset/data/attention_step1_run0004.npz` |
| 사용한 셀 | **L8 h2**, Euler 0, `last` pooling (1단계가 고른 것을 그대로 이어받음) |
| target | `apple` |
| 검사 프레임 | 20개 × 카메라 3대 (순위 지표는 target이 200 px 이상 보이는 프레임만: cam_high 14개) |
...
| B. 픽셀 대응 | 통과 — nearest 오차 0.00e+00, bilinear 범위 이탈 1.14e-13 | < 1e-6 |
| C. 정규화 순위 보존 | 통과 — 단조성 240/240, 기본값 경로 seed 집합 완전 일치 | 단조성 전부 + percentile·minmax·none 완전 일치 |
| D. 순위 보존 (head) | 실패 — peak **0.214**, AUC **0.98104**, precision@N **0.182** | peak >= 0.8, AUC >= 0.9 |
...
나가면 안 된다 — 보간은 섞을 뿐 만들어내지 않는다. 범위 이탈 최대 **1.14e-13**.
...
| softmax | 0.9891 ~ 1.0000 | 아니오 |
| none | 1.0000 ~ 1.0000 | 예 |

**`softmax`만 갈린다.** 최소 Jaccard 0.9891 — 약 2,600개 seed 중 최대 수십 개다.
...
|---|---|---|---|---|
| cam_high | 0.214 | 0.98104 | 0.182 | 26 |
| cam_left_wrist | 0.211 | 0.74700 | 0.145 | 30 |
| cam_right_wrist | 0.455 | 0.98407 | 0.293 | 27 |

![순위 지표](../asset/image_004/lifting/fig1_ranking.png)
...
|---|---|---|---|---|
| cam_high | 0.0034 / 0.0037 → **0.87** | 0.0064 / 0.0073 → **0.75** | 0.0131 / 0.0183 → **0.60** | 0.0187 / 0.0365 → **0.42** |
| cam_left_wrist | 0.0028 / 0.0055 → **0.55** | 0.0050 / 0.0109 → **0.49** | 0.0109 / 0.0273 → **0.42** | 0.0195 / 0.0545 → **0.38** |
| cam_right_wrist | 0.0030 / 0.0031 → **0.96** | 0.0056 / 0.0062 → **0.90** | 0.0120 / 0.0156 → **0.79** | 0.0203 / 0.0311 → **0.67** |
...
**0.75** — 순도 0.0065라는 숫자가 실패가 아니라 크기
때문임을 이 한 값이 보여준다.
...
|---|---|---|---|---|
| cam_high | 346 | 9668 | 43880 | 1797 |
| cam_left_wrist | 267 | 1606 | 50233 | 84 |
| cam_right_wrist | 261 | 1053 | 44478 | 1 |
...
![seed 점의 위치](../asset/image_004/lifting/fig2_seed_points.png)
...
|---|---|---|---|
| `cam_high` | **0.214** | 0.98104 | 0.182 |
| `cam_left_wrist` | **0.211** | 0.74700 | 0.145 |
| `cam_right_wrist` | **0.455** | 0.98407 | 0.293 |
