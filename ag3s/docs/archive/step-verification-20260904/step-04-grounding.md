...
|---|---|---|---|---|---|---|
| 0 | 51,878 | 15.9 mm | 9 | 0 | 0 | `no_cluster` |
| 11 | 59,029 | 15.9 mm | 9 | 0 | 0 | `no_cluster` |
| 22 | 59,289 | 15.9 mm | 9 | 0 | 0 | `no_cluster` |
| 33 | 54,817 | 15.9 mm | 9 | 0 | 0 | `no_cluster` |
...
| 검사 | 결과 | 기준 |
|---|---|---|
| A. 옳은 물체 선택 | 통과 — 채점 가능 8프레임 중 **8개** | 전부 |
| B. 클러스터 품질 | 통과 — IoU **0.953**, 정밀도 0.953, 재현율 1.000 | IoU >= 0.5 |
...
**채점 가능성 기준.** 파지 전 9프레임 중 8개를 채점하고
...
제외된 유일한 프레임은 target이 정확히 20점, 즉 `min_points`와 같은 지점이다. 클러스터가 겨우 성립할 수 있는 이론적 경계이고, 실제로 성립하지 못했다.

채점 가능한 프레임들의 점 수는 146–256점으로, 경계와 확실히 떨어져 있다.
...
가려진 16프레임에서 `ground_target`은 `ok`와 함께 다른 물체를 반환한다. attention이
...
|---|---|---|---|---|---|---|---|
| 1 | apple | target | **0.8930** | 0.9905 | 0.5865 | 0.0281 | 256 |
| 2 | banana | 다른 물체 | **0.4634** | 0.4565 | 0.6292 | 0.1007 | 151 |
| 3 | crate | 다른 물체 | **0.0144** | 0.0009 | 0.0542 | 0.3251 | 4410 |
| 4 | table | 배경 | **0.0128** | 0.0180 | 0.0001 | 0.4895 | 3662 |
...
상위 5위 안에 배경 클러스터가 나타난 횟수: **26회** (순위 [2, 3, 4]).
