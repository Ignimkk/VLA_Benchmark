...
`self_filter_inflation` 을 **0.05 mm 로 올리는 수정 진행 중**이다 — 베이스 누수를 줄이되
끝단의 유용한 점군은 버리지 않기 위한 조정이다.

### 다음 — 항목 1 과 4 의 수정 대기

1. **항목 1**: target_score 의 에피소드별 보정안 필요 — T2 에서 다룬다 (pick-place 상태 전이와 함께)
2. **항목 4**: self_filter_inflation 수정값 확정 후 회귀 기준선 재측정

**T0~T3 중 회귀는 이미 확보됐고** (기준선 기록 1 개 · 수정 후 기록 1 개), R 도 대기 목록에서
벗어난다. **T2(파지 상태) · T3(TSDF/ESDF 나이)를 마저 열면 R을 집고 넘어간다.**
