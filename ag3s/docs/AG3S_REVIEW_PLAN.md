...
> **최종 갱신 2026-09-25 — 16D 통합 테스트(T0~T6) 진행 중. 최근 발견: 로봇 마스크 수정(self_filter_inflation·gap-filling capsule) · T2 자산 확보(ep1807) · X2(머리 카메라) · X1(고정 구 분할) · T1-a(배선 결함)**
>
...
>
> **실행 기록의 정본은 [AG3S_T0T6_LOG.md](AG3S_T0T6_LOG.md)** 다 (2026-09-24 부터).
> 그 앞의 모듈 검토 국면은 [AG3S_REVIEW_LOG.md](AG3S_REVIEW_LOG.md) 에 있고 이어 쓰지 않는다 —
> 거기에 스텝별 진행 현황, 누적
...
## 다음 작업 — T0~T6 전체 통합 테스트 (2026-09-22 착수)

`AG3S_TOTAL_TEST_Prompt.md` 를 받아, 실행 계획을
[`AG3S_CUROBO_LIVE_TEST_PLAN.md`](AG3S_CUROBO_LIVE_TEST_PLAN.md) 의 **"T0~T6 전체 통합 테스트 —
구현 및 실험 계획"** 절에 세웠다. 프롬프트 §6 이 새 통합 실험 계획 MD 를 만들지 말라고 하므로
그 문서 안에서 끝낸다. 이 절은 **검토 계획서 쪽에서 본 위치**만 적는다.

**무엇이 바뀌는가** — 이 검토는 지금까지 모듈을 하나씩 판정하고, 마지막에 그것들이 한 실행
안에서 실제로 도는지를 확인했다(2026-09-18). T0~T6 은 그다음이다: **기존 기록의 재생을
입력에서 빼고**, 새 씬·새 seed·실측 정책 attention 으로 같은 파이프라인을 처음부터 다시
돌린다. 그래서 지금까지의 통과 판정이 재검증 대상으로 내려간다.

**게이트 하나가 이 검토의 열린 항목과 겹친다** — 프롬프트 T0 의 즉시 실패 조건이 "legacy
backend 호출" 이고, 현재 `pipeline._build_esdf` 는 legacy `EsdfBuilder` 를 부른다. 즉 위
"순서" 절의 4 번(배선)이 **T0 의 전제조건**이 되었다. 계획서 쪽에서 미뤄 둔 이유(Step 10 이
필드 내용을 바꾸므로 두 번 반영하게 된다)는 Step 10 이 닫혀 사라졌다.

**확정한 방향 셋** (사용자 판정 2026-09-22):

1. **단일 프로세스** — openpi venv 복제본에만 warp+curobo 를 넣는다. 위 "venv 장벽" 정정 참고
2. **`--seed` 신설** — `pi05_infer.py` 에 seed 를 넣고 layout·slot order·target jitter 를 파생
3. **실시간성 분리 보고** — 기능 통과와 533 ms 예산 초과를 따로 기록. "실시간성은 범위 밖"
   (2026-09-07) 결정 유지

![T0~T6 계획 지도](figures/live-test/t0-t6-plan-map.png)

**진행** — I1(단일 프로세스 성립 확인) **통과**(2026-09-22). openpi venv 복제본에서 cuRobo 가
정책·MuJoCo·SQP 와 한 프로세스에서 돌고, 새 씬 seed 3개에서 coarse eikonal 0.9995 ·
fine 0.9999 · 거리장 비용 약 4.3~4.8 ms. 원본 venv 무오염, 회귀 기준선·테스트 625 불변.
수치와 실패 기록은 `AG3S_CUROBO_LIVE_TEST_PLAN.md` 의 "I1 실행 결과" 절과
`AG3S_REVIEW_LOG.md` 의 "I1" 절.

**I2(backend 교체) 통과**(2026-09-22). 위 "순서" 절의 4 번이 이것이다 — `_build_esdf` 가
`CuroboFieldBuilder` 를 부르고 legacy `EsdfBuilder` 생성이 0 회다. 쥔 물체는 파내기 대신
**seed 제외**로 다루고(TSDF 를 안 다치고 second-nearest 환경 장애물 거리를 유지한다), 라벨
층은 cuRobo 의 `site_index` 조회다(실측 오차 중앙 0.10 mm). 판정이 갈리는 근접 띠에서 두
backend 가 2 mm 안에서 일치하고, legacy 가 위반으로 본 구를 cuRobo 가 완화한 경우는 0 이다.
회귀 기준선·테스트 625 불변. 그 과정에서 검사 넷(coverage 격자 · 죽은 카메라 · 미관측 비율 ·
관측 판정식)이 조용히 죽어 있던 것을 고쳤다 — 수치는 로그의 "I2" 절.

**cuRobo 별도 기준선 확정**(2026-09-22) — 15 청크에서 해소 11 / 개선 15 / feasible 9,
violated 6. legacy(해소 14 · feasible 8)는 고정. 해소 14 → 11 은 회귀가 아니라 0 근처
±4.5 mm 띠에서 이진 딱지가 갈린 것이고, 대조군으로 돌린 cuRobo **단일 계층**이 해소 15/15 로
더 좋아 보이는 것은 2계층보다 중앙 **+3.36 mm 낙관**이기 때문이다 (12/15 프레임) — C5(거친
20 mm 가 판정 지점에서 +7.56 mm 낙관)와 같은 크기이고, D2("미세 계층은 값어치를 안 한다")를
뒤집은 판정이 live 경로에서 재확인됐다. TO 70 → 427 ms 는 격자 복셀 수(547 k → 4.19 M)를
따르고 계층 수가 아니다 — 단일 계층에서도 364 ms.

**I3(field provenance 와 age) 통과**(2026-09-22). `FieldProvenance` 를 두 backend 가 **같은
모양으로** 찍고(키 14 개), `wire` 응답에 `field` 블록으로 실린다. 상태는 두 곳에서 갈린다 —
서버는 `new`/`unavailable`, `carried`/`stale` 은 청크 하나가 덮는 8 개 control frame(533 ms)
에서 클라이언트가 채운다. age 의 기준은 `observed_at`(가장 최신 촬영 시각, 클라이언트 시계)
이고 서버 monotonic 은 원점이 달라 쓰지 않는다.

**`max_field_age_sec` 기본값을 `None` 으로 비워 뒀다** — F14(상태 지연 한계 100 ms 가 측정
없이 정해졌고 실측하니 피해가 16 ms 에서 시작했다)를 반복하지 않기 위해서다. 한도가 없으면
`stale` 로 올리지 않고 `staleness_checked: false` 를 실어, 검사를 안 한 것이 통과한 것으로
읽히지 않게 한다. 한도는 T3 에서 프레임 간 필드 변화량을 재고 정한다.

**I4(seed · 프레임 기록) 통과**(2026-09-22). `FrameRecorder` 가 manifest(불변 설정) +
frames.jsonl(observation/planning/control) + completeness.json 을 낸다. `--seed` 는 layout
(2~15) · slot order · xy jitter 만 파생시키고 난도는 `--obstacle-profile` 이 명시한다.
주입 시험이 T0 의 즉시 실패 조건(중복 일련번호 · timestamp 역전)을 실제로 잡는다.

**그리고 T5·T6 의 전제조건이 막혔다** — `pi05_infer.py` 가 `mink`(MuJoCo IK)를 import 시점에
요구하는데 네 venv 어디에도 없다. RUNBOOK 이 그 driver 를 "로컬" 이라고 적은 그대로이고, 이
서버에서는 한 번도 안 돌았다. `benchmark` 의 `TransportScene` 은 다른 클래스이고 `mink` 를
안 쓰므로 I1~I4 는 영향이 없다. **판정 결과 `mink 1.3.0` 을 `.venv-openpi-live` 에
넣어 해소했다** — core 핀 버전 전부 불변, 원본 venv 무오염, `.venv-curobo` 무사. 그리고
`--seed` 가 실제 씬을 바꾸는 것을 실측했다 (seed 101 과 202 의 apple 이 87 mm 차이, 같은
seed 는 비트 단위 동일).

**I1~I4 가 끝났다.** 이제 `.venv-openpi-live` 하나에 정책(jax CUDA) · 시뮬(mujoco) ·
IK(mink) · 지각(warp·curobo) · 최적화(casadi·osqp) 가 다 있고, T0 을 시작할 수 있다.

**T0(환경·배선) 통과**(2026-09-24) — 16D held-out test split 에서 양팔 × 네 과일로 에피소드
4 개를 48 제어 스텝씩 새로 굴렸다. 8 항목 전부 통과: 프레임 54/54 의 출처 도장이 `curobo`,
기록 완결성 4/4(누락·중복·역전·설명 안 된 carried 전부 0), 청크 폭이 설정 문자열이 아니라
**온 것의 폭**으로 16 이다. **legacy 생성 0 을 추론에서 불변식으로 바꿨다** — 출처 도장은
*쓰인* 필드만 말하므로 `EsdfBuilder.instances_created` 를 세고 프레임마다 검사한다.

그 과정에서 **기록이 사실을 덜 말하던 자리 넷**을 고쳤다: 관측 프레임이 한 번도 안 불렸고,
기대 관측 수를 제어 스텝 수로 잡아 일어나지 않은 관측이 영구 누락으로 남았고(관측은 정책
호출당 한 번이다 — **기대 쪽이 틀린 경우**), 카메라 촬영 시각이 호출부에 도달하지 않았고,
실제 청크 폭과 에피소드 배치가 기록에 없었다.

**실시간성은 별도 판정으로 실패다** — 24 청크에서 서버 total P50 2519 ms, 청크 예산 533 ms 의
**4.7 배이고 24/24 가 초과**했다. 지배 항은 AG3S 1943 ms(서버 시간의 77 %)이고 TO 는 279 ms 다.
control 프레임이 쓰는 필드가 **2.6 초 낡았다** — `carried` 는 설계대로 돌고 나이도 정직하게
실려 있지만, 이 수치가 `max_field_age_sec` 를 T3 에서 정할 때의 근거다. 위 "확정한 방향 셋"
의 3 번(실시간성 분리 보고)이 여기서 처음 실제로 적용됐다.

**남은 두 물음** — (1) AG3S 1943 ms 의 단계별 내역(기록에는 합계만 있다, T3), (2) 같은
에피소드를 두 번 굴렸을 때 판정 수가 달랐다(safe 3/3 대 5/1; 정책 노이즈가 유력하나 재지
않았다, T1).

**회귀 기준선을 16D 로 갈았다**(2026-09-24). T0 뒤 기준선을 돌리자 **죽었다** —
`run_0004` 의 청크는 14 열이고 `wire.ACTION_WIDTH` 는 16 이다. 16D 전환의 결과로 CLAUDE.md
의 안전망이 실행 불가가 된 것이고, 전환 작업에서 같이 갈지 않은 빠진 자리였다. 새로 세웠다:
held-out ep1800 을 평범한 정책 서버로 굴려 기록(`run_16d_ep1800`)을 만들고, 같은 체크포인트로
attention 을 추출하고(`attention_16d_ep1800.npz`), `--cameras all` 로
**해소 13 / 개선 15 / feasible 13, violated 2** (4 회 반복 동일). 옛 head-only 관례는
`run_0004` 씬에 묶여 있어 에피소드 기반 씬에서는 시작 위반이 0/15 — 고칠 것이 없어 기준선
구실을 못 한다. **개수까지만 인용한다**: `sqp.time_budget_ms` 가 벽시계 마감이라 프레임별
여유거리와 반복 수가 부하를 따른다 (CLAUDE.md 가 예전부터 개수만 인용해 온 이유가 여기서
처음 설명됐다).

**그리고 T1 의 첫 과제가 정해졌다** — 이 기준선은 **attention → target grounding 을 재지
않는다.** 오프라인에서 `has_target` 이 15/15 프레임 false 인데 live 경로는 같은 에피소드에서
성공한다(T0 에서 AG3S `ok` + 미세 계층이 붙었다). 차이는 오프라인이 **고정된 셀 선택**(층·
헤드·agg·denoise, 14D 모델에서 정해진 L8H2)을 쓰는 것이다. 사용자가 "이전 테스트도 새 모델로
다시 해보라" 고 한 것이 정확히 이 지점이고, T1 에서 셀을 쓸어 grounding 성공률로 판정한다.

> ~~차이는 셀 선택이다~~ — **기각됐다** (`T1-a`, 2026-09-25). 1296 조합 전수에서
> `peak_on_target` 이 **0** 이다. 진짜 차이는 **attention 을 어느 카메라에 붙이느냐**이고,
> 사과는 `cam_high` 에 **한 프레임도 안 찍힌다**. 아래 "T1-a" 문단.

*덤*: 평범한 정책 infer 가 **0.09 초**다 — serve_safe 경로 2725 ms 와 비교하면 2.7 초가
전부 AG3S+TO 라는 것이 독립적으로 확인된다.

**T1 앞에 R(재측정 7건을 16D 로)을 끼웠다**(2026-09-25 착수). 로그의 **"재측정 대기"** 표
일곱 건 — I2(backend 대조) · cuRobo 별도 기준선 · 쥔 물체 부호 교정 문턱 · C5(거친 20 mm 의
낙관) · C3(미세 창 경계 불연속) · N1·N2·F20(잠복 값) · F14(상태 지연 피해 시작점) — 은
**결정은 유효한데 수치가 14D 기록에서 나온 것**이다. T1~T6 의 게이트 판정이 그 수치를
전제로 쓰므로, 전제가 14D 인 채로 통과시키면 통과 기준을 낮춘 것과 같아진다. 그래서 순서를
T1 앞으로 당겼다.

첫 걸음은 수치가 아니라 **분류**였다 — 일곱 스크립트를 고치지 않고 그대로 16D 긴 기록
(`outputs/live_test/20260924_long16d/run_0000/`, 정책 호출 50 · 제어 392 스텝, 파지 포함)에
겨눠 `runs`/`broken`/`needs-arg`/`needs-outpath` 로 셌다. 회귀 기준선은 개수 5/5 일치.

**1 차 결과 — 분류는 `runs` 4 · `broken` 1 · `needs-arg` 1 · `needs-outpath` 1 인데, 쓸 수
있는 수치는 7 중 1 건이었다**(2026-09-25). 무효였던 이유는 전부 **스크립트가 16D 를 안
먹었기 때문**이지 값이 틀려서가 아니다 — R1·R3 은 `--records` 가 없어 seed 로 새 씬을 만들고,
R4 는 자세만 16D 이고 관측·필드는 2026-09-12 자 `/tmp` npz 인 혼합 실행이었으며(스크립트가 그
불일치를 안 잡는다), R5 는 argparse 가 없어 2026-09-11 자 프레임만 읽는다. **`runs` 는
프로세스가 0 으로 끝났다는 뜻이지 새 기록을 쟀다는 뜻이 아니다** — 이것이 이 라운드가 실제로
답한 것이고, 분류를 수치보다 먼저 센 덕분에 이식 비용을 측정 결과로 착각하지 않았다.

**16D 로 닫힌 것은 R6 의 N2 하나**: 해석적 채널을 끄면 `arms` 120 구에서 최대 낙관
**+258.2 mm**(중앙 +47.7, 낙관 질의 1241), 켜면 **0**. `all` 194 구는 +408.0 mm → 0.
14D 의 +219.5 → +0.0 mm 와 **결론이 같다** — 채널을 켜면 낙관이 완전히 0 이다.

**2 차에서 더 깊은 것이 나왔다 — R4·R5 는 이식으로도 안 풀린다.** 실측 attention npz
(`attention_16d_long.npz`)를 붙여 R2 를 다시 돌렸는데 **수치가 한 자리도 안 바뀌었다**:
legacy 는 실측·합성 둘 다 `feasible 12`, cuRobo 는 둘 다 `7`, 그리고 **네 실행 전부
`has_target` 0/15**. attention 이 grounding 까지 도달하지 못한다는 뜻이고, 그 결과
`rollout_fields_16d.npz` 의 키가 **`coarse 45 / fine 0`** — **미세 5 mm 계층이 한 프레임도
안 붙는다**. 미세 계층은 grounding 된 target 중심에 놓이므로 grounding 이 없으면 생기지
않는다. **R4(C5)·R5(C3)는 견줄 2계층 쪽이 없어 이 기록에서는 잴 수 없고, 경로를 인자로 빼도
못 잰다** — 막는 것이 스크립트가 아니라 앞단이다.

재현성도 열려 있다. 같은 명령 4 회에서 legacy `feasible` 이 **12·7·12·12** 로 1 회 이탈하고
(앞선 `12·7·7·12·12` 는 `--dump-frames` 실행을 섞은 집계 오류로 정정됐다), SQP 반복 중앙값은
다섯 표본 전부 2 라 "반복 수가 줄어서" 로 설명되지 않는다. `run_16d_ep1800` 기준선은 4 회
전부 같았고 **긴 기록에서만** 흔들린다. **원인 미상 · 확정 전**이다.

**사용자 판정 다섯**(2026-09-25): (1) R 을 **R-port(이식) / R-measure(측정)** 로 쪼갠다,
(2) R1 은 **T0 불변식을 건드리지 않고** 프로세스를 나눠 푼다 — legacy 로 한 번, cuRobo 로 한
번 따로 돌려 npz 로 내고 대조는 밖에서 한다(불변식을 비교 편의로 푸는 것은 안전망을 재료로
쓰는 것이다), (3) R7 은 `step7_state_lag.py` 의 `STEP_MS` **단위를 확정한 뒤** 다시 잰다 —
옛 값을 다시 계산해 바꾸지 않고 "14D 는 옛 단위, 16D 는 새 단위" 를 표에 명시한다.
**(4) T1 의 첫 과제인 attention 셀 선택을 R 앞으로 당긴다** — R4·R5 가 거기 막혀 있고, 셀
선택은 R 의 수치를 하나도 쓰지 않으므로 순환이 아니다. **(5) 재현성은 N 회 반복으로 덮지
않고 원인을 추적한다** — 다수결을 취하면 흔들린다는 사실이 평균 안에 숨는다.

**그래서 순서가 `T1-a` → `R-port` → `R-measure` 가 됐다.** `T1-a` 는
`layer × head × denoise × aggregation` **1296 조합 전수 sweep** 으로 grounding 성공률을 내고,
14D 에서 정한 `L8H2` 가 16D 에서도 서는지 · 16D 최선이 무엇인지 · **live 서버가 실제로 고르는
셀과 같은지**를 답한다 (오프라인은 실패하는데 live 는 같은 에피소드에서 성공하므로 여기 답이
있을 가능성이 높다). `R-port` 는 `T1-a` 와 독립으로 시작할 수 있다 — 막힌 것은 R4·R5 둘뿐이다.
계약은 `handoff/T1-a.task.md` 와 `handoff/R-port.task.md`, 수치와 분류는
[`AG3S_T0T6_LOG.md`](AG3S_T0T6_LOG.md) 의 **"R — 재측정 7건을 16D 로"** 절.

**그리고 사용자 판정이 R 의 성격을 바꿨다**(2026-09-25): *"14D 랑은 비교하지 마. 14D 기록은
모두 파기하고 16D 로 새로 측정하고, AG3S 도 16D 를 맞춰서 수행할 거야."* 물음이
*"14D 값이 16D 에서도 같은가"* 에서 **"16D 에서 이 값은 얼마인가"** 로 바뀌었고, 로그의 값
표에서 `14D 값`·`달라졌나` 두 열을 없애 **`16D 값 │ 어떻게 쟀나 │ 표본`** 으로 갈았다.
**14D 값이 틀려서가 아니다** — 모델·기록·action layout(`ACTION_WIDTH` 14↔16,
`ARM_JOINT_DIM` 6↔7) 셋이 한꺼번에 달라, 나란히 놓으면 차이가 어느 축에서 왔는지 아무도
답할 수 없기 때문이다. 내려온 것은 *기준으로서의 자리*이고 **archive `AG3S_REVIEW_LOG.md`
는 고치지 않는다.** 판정 (6) 14D 와 비교하지 않는다 · (7) **고정 구 이상(X1)을 즉시
조사한다.** 계약은 `handoff/R-measure.task.md`.

**X1 — 고정 구가 42 개다(2026-09-25, 조사 중).** N1 측정 스크립트를 돌리다 나왔다. 16D 에서
`arm_6`(손목)이 자유 관절이 됐으니 고정 구(= 최적화가 움직일 수 없는 로봇 구)는 **줄어야**
맞는데 늘었다 — 방향이 반대다. **고정 구 / 움직일 수 있는 구의 분할이 틀리면 그 위에서 도는
모든 여유거리 계산이 함께 틀린다 — 회귀 기준선까지 포함해서.** 확장으로 X2(AG3S 안에 관절
수·action layout·링크 목록·구 목록이 14D 를 전제하는 자리를 `파일:줄` 로 훑는다).

**`R-port` 이식은 P5 를 빼고 끝났다**(2026-09-25, 테스트 **648 passed** = 기존 625 + 신규 23).
가장 말할 만한 것 셋: **P6** 단계 경계를 그리퍼가 닫히는 시점에서 파생하게 고쳤는데 그 규칙을
14D `run_0004` 에 적용하면 박혀 있던 상수 `(24, 56, 72)` 가 **그대로 재현된다** — 옛 상수는
규칙이 아니라 한 기록에서 나온 답을 굳힌 것이었다. **P7** `STEP_MS = 16.0` 을 없애고 기록
`meta` 에서 파생하게 했다(한 장 = 8 제어 스텝 = **533.3 ms**; 옛 16.0 은 `8 × 2 ms` sim
timestep 으로 곱할 상대를 잘못 잡은 값이다) — 그 결과 **F14 의 옛 결론이 성립하지 않는다**:
새 단위에서는 0 이 아닌 어떤 지연도 설정 한계(100 ms) 밖이라 판정은 **"이 기록의 해상도로는
한계 안쪽을 못 잰다"** 가 된다(답하려면 제어 스텝 단위 기록이 필요하다). **P3** 생산자가
출처 도장을 찍고 소비자가 세 인자의 짝을 확인해 **안 맞으면 즉시 실패**한다 — 1 차의 혼합
실행(자세만 16D)이 이제 통과하지 못하고, 도장 없는 옛 형식도 통과시키지 않는다.

### T1-a — 셀이 아니라 **배선**이었다 (2026-09-25)

**이 계획서가 2026-09-24 에 세운 짐작이 기각됐다.** *"오프라인이 실패하고 live 가 성공하는
차이는 고정된 셀 선택(`L8H2`)"* 이라고 적었는데, `attention 셀` **1296 조합**
(`denoise 3 × agg 3 × layer 18 × head 8`)을 전수로 쓸어 보니 `peak_on_target` 이
**ep1800 0/15 · long16d 0/50**, `target_mass_mean` 은 **정확히 0.0** 이다.
**`L8H2` 가 안 맞는 것이 아니라 어느 셀도 안 맞는다 — 셀 축에 답이 없다.**

**진짜 원인**: grounding 은 `cam_high` 의 attention 만 읽는데(`esdf_rollout.py:137`)
**사과가 그 카메라에 한 프레임도 안 찍힌다** (`zed_left` 0/15 · 0/50 프레임). 사과는
**왼 손목 카메라**에만 보인다(15/15 중앙 2,938 px · 42/50 중앙 4,178 px). 그리고 오프라인은
attention 맵을 **`zed_left` 한 대에만** 붙이는데(`esdf_rollout.py:266`) live 는 **세 대에
각각** 붙인다(`serve_safe.py:162-163` → `wire.py:171`). **오프라인 ↔ live 차이가 이것이다** —
파이프라인 결함이 아니라 **하네스가 서빙 경로와 갈라져 있던 것**이다.

**새 결함 — grounding 이 실패를 성공으로 보고한다.** 14D 셀로 긴 기록을 돌리면 `has_target`
**31/50** 인데 `on_apple` 은 **0/50** 이고 지목한 중심이 사과에서 **0.36~3.59 m** 떨어져
있다. `LOW_SCORE` 가 19 프레임에 찍혔는데도 `has_target` 은 참이다. **`has_target` 은
"후보를 하나 골랐다" 일 뿐 "그것이 조작 대상이다" 가 아니다** — 앞으로 grounding 성공률에는
`on_target` 을 반드시 함께 센다.

**판정 (8)** 오프라인을 live 와 같게 고친다 — 세 카메라에 각각 attention 을 붙인다.
**서빙 경로를 기준으로 삼는다.** **(9)** `cam_high` 가 대상을 한 번도 못 보는 것을 **AG3S
설계 문제로 올린다** — grounding 이 카메라 한 대에 의존하는 구조가 옳은가. 조작 대상은 팔이
다가갈수록 머리 카메라에서 가려지고 손목 카메라에서 커지는데, 지금 구조는 그 반대다.

**재현성도 절반 닫혔다**: 뒤집힌 것은 **판정이 아니라 mm** 이다. 프레임 5·11·12·13·14 가
**한 실행에서만** 뒤집히고, 두 실행은 `arm_after_mm` 가 소수 셋째 자리까지 같아 **계산은
결정적**이며, 폭이 최대 **35.52 mm** 라 0 근처에서 갈린 것이 아니다(안 뒤집힌 프레임 중에
`|clearance|` 0.119 mm 인 것이 있다). **원인은 여전히 미상이다.**
...
**환경 사실 정정** — venv 는 둘이 아니라 **셋**이다. `.venv-openpi-live`(py3.11,
numpy 1.26.4)에 mujoco 3.11.0 · casadi 3.8.0 · osqp 1.1.3 · torch 2.7.1 · curobo 0.8.0 ·
warp 1.17.0 이 전부 있다 (2026-09-25 직접 import 로 확인, CLAUDE.md 수정됨). *"numpy 때문에
합칠 수 없다"* 는 옛 서술이고, 그것을 전제로 실패를 읽으면 스크립트 결함을 환경 탓으로
오판한다 — 1 차의 `broken`/`needs-*` 셋을 이 venv 로 다시 돌렸고 **재분류 0 건**이었다.

### 쥔 물체 처리 — cuRobo 경로를 고쳤다 (2026-09-22, 사용자 질문에서 잡혔다)

사용자가 "쥔 물체를 어떤 방법으로 처리하나" 를 묻는 과정에서 **I2 의 seed 제외가 목적과
반대로 동작한다는 것이 실측으로 잡혔다.** cuRobo 는 부호를 질의 복셀의 TSDF 에서 가져오므로
(`builder_esdf.py:455-489`) seed 만 지우면 *더 먼 거리에 음수 부호*가 붙는다 — 쥔 물체
질의점 184 개에서 음수가 38 → **59**, 최소가 −12.93 → **−33.07 mm** 였다.

고친 것은 **순수 복셀만 부호 교정**이다 (사용자 판정 B). 쥔 물체 말고는 아무것도 없는 복셀
에서만 부호를 양수로 강제하고, 환경 표면도 든 복셀은 그대로 둔다 — 숨길 수 있는 관통이
경계값이 아니라 **구조적으로 0** 이다. 순수 판정은 추가 데이터 없이 되고(seed 제외 뒤의
거리가 문턱보다 크면 그 자리에 다른 표면이 없다), 문턱 **1.5 복셀**은 `run_0004` 의 실제 파지
구간에서 쓸어 정했다 — 파지 직후 공유 157 · 운반 **0** · 담기 26 이고, 세 문턱의 trajopt
판정이 프레임 단위로 같아 문턱이 **숨김 위험만** 바꾼다는 것이 확인됐다.

**로봇 쪽 편입은 바꾸지 않았다** — cuRoboV2 의 발상대로 쥔 물체를 로봇에 편입하되
primitive 가 아니라 **점 기반**이다(F19). cuRobo v1 부록 D 의 구 근사는 이미 기각된 것이다.

그 과정에서 **하네스 결함 하나**도 잡혔다: 사과의 점만 옮겨 만든 합성 "들어올림" 배치는
TSDF 증거가 원래 자리에 남아 **빈 공간을 잰다.** 파지 기전을 재려면 잠금이 `attach()` 를
부르는 실제 구간이어야 한다. 수치는 `AG3S_REVIEW_LOG.md` 의 "쥔 물체 처리" 절.
...
**기록 파일**: `benchmark/ag3s/docs/AG3S_T0T6_LOG.md` — 한국어.
(2026-09-24 이전은 `AG3S_REVIEW_LOG.md`, archive.)
...
**규칙 C — 용어를 설명한다** (2026-09-12). 기록에 쓰는 말은 `AG3S_T0T6_LOG.md` 의 **"용어"**
...
> **정정 (2026-09-22) — 세어야 할 venv 가 하나 빠져 있었다.**
> 위 두 줄은 `.venv-ag3s` 와 `.venv-curobo` 만 센 것이다. 그런데 **`serve_safe.py` 가 실제로
> 도는 venv 는 둘 중 어느 것도 아니다** — `pi05_TO_hybrid/openpi/.venv` 이고, 그것은
...
> `curobo` 둘뿐이다.
>
> 즉 "`.venv-curobo` 에 casadi·osqp 를 넣는다" 가 아니라 **openpi venv 에 warp+curobo 를
> 넣는다** 가 더 짧은 길이다. `warp-lang 1.17.0` 은 `py3-none-manylinux_2_28_x86_64` 휠
> (`requires_python >= 3.10`)이고 cuRobo 는 `warp-lang >= 0.10` + `torch >= 2.5` 만 요구하며
> CUDA 확장은 선택(`USE_PYBIND` 기본 0)이다.
>
> 정책을 서빙하는 venv 를 건드리지 않기 위해 **복제본(`.venv-openpi-live`)에만 넣기로**
> 판정했다 (2026-09-22). 실행 계획과 통과 조건은
> [`AG3S_CUROBO_LIVE_TEST_PLAN.md`](AG3S_CUROBO_LIVE_TEST_PLAN.md) 의 "T0~T6 전체 통합 테스트"
> 절 I1 에 있다.
...
1. ~~**[선행]** `.venv-curobo` 에 casadi · osqp 설치 가능성 확인~~ — **불필요로 닫힘
   (2026-09-22).** 위 정정대로 openpi venv 가 이미 casadi·osqp·mujoco·torch 를 갖고 있다.
   선행 확인의 대상이 "`.venv-curobo` 에 casadi·osqp 가 되는가" 에서 "openpi venv 복제본에
   warp+curobo 가 되는가" 로 바뀌었다
2. Step 7 · 8 · 9 (검토 계속) — **완료**
3. **Step 10** — 쥔 물체가 필드/로봇 모델 중 어디로 가는지 확정 — **완료** (점 기반, E3·F19)
...
## 사용자 판정 셋 (2026-09-24) — 기록을 나누고, 수치를 새 모델로 다시 잰다

T0 이 통과한 직후 사용자가 네 가지를 물었고 셋이 판정으로 확정됐다.

| 물음 | 판정 | 결과 |
|---|---|---|
| AG3S 를 온라인으로 돌릴 예정인가 | 맞다. 다만 두 뜻을 갈랐다 — **프레임마다 새로 관측해 필드를 다시 짓는 것은 이미 그렇게 돌고 있고**(T0 이 그것), **실시간 예산 안의 닫힌 루프는 아직**이다(4.7 배 초과, T5·T6) | — |
| 8123 정책 서버를 종료해도 되나 | **된다.** 8810 MiB 를 쥐고 있고, 대체 경로가 둘 다 실재한다 (`serve_safe --no-safe` · `pi05_infer --checkpoint` 로컬 로드). 필요한 기록은 종료 전에 다 떴다 | 사용자에게 맡김 |
| 모든 테스트를 새 모델로 | **가능하고, 일곱 개 전부 지금 다시 잰다** | 아래 |
| 로그를 나눌까 | **나눈다** — `AG3S_T0T6_LOG.md` 가 새 정본 | 아래 |

### 기록을 나눴다 — 규칙 D 와 부딪히므로 규칙도 같이 고쳤다

옛 로그는 514 KB · 74 절이고 **모듈을 하나씩 판정하던 국면**의 기록이다. 지금은 입력(저장된
기록 재생 → held-out 에피소드)·판정 기준(모듈별 결함 → 프레임별 증거의 완결성)·목표(모듈이
옳은가 → 실기 닫힌 루프가 서는가)가 모두 다른 국면이다.

규칙 D 가 막으려던 것은 *"기록이 갈라져 어느 쪽이 최신인지 모르게 되는 것"* 이므로 파일만
새로 만들지 않고 넷을 같이 했다.

1. 옛 `AG3S_REVIEW_LOG.md` 머리에 **"더 이상 이어 쓰지 않는다 → 새 로그"** 표지를 박았다.
2. **용어 표 95 항목을 새 로그로 옮겼다** — 규칙 C 가 "새 용어는 로그의 용어 절에" 라고 하므로,
   용어가 옛 파일에 남으면 이어 쓰지 않기로 한 파일에 계속 쓰게 된다.
3. 오늘 쓴 세 절(T0 · 16D 회귀 기준선 · 왜 8000 인가)을 새 로그로 옮겼다.
4. 규칙 본문 **두 곳**(`CLAUDE.md` 와 `.claude/ag3s-rules.md`)의 읽는 순서·규칙 C·규칙 D 를
   갱신하고, 다른 문서에서 옛 로그를 *정본*으로 가리키던 곳 7 군데를 돌렸다. 과거 측정을
   가리키는 참조는 그대로 뒀다 — 그것은 archive 가 맞다. 끊어진 링크 0.

**다음에 또 나누지 않는다.** 그 문장을 규칙 D 안에 박아 두었다.

### 아키텍처 — T1~T4 는 서버 쪽 AG3S 유지, T5·T6 에서 옮긴다

실기에서 AG3S 가 로봇 쪽에서 온라인으로 돌면 목표 구성은 `--trajopt` in-process 이고 지금의
`--safe-remote`(서버 쪽 AG3S)가 아니다. 그런데 **T0~T4 가 프레임별 IPC 기록을 판정 항목으로
요구하고, 한 프로세스 안에서는 IPC 가 아예 없어져 그 항목이 "해당 없음" 이 된다.** 그래서
T1~T4 는 지금 구성을 유지하고 T5(shadow)에서 옮기며, 그때 두 구성의 지연을 나란히 잰다.
전환 신호와 대조표는 새 로그의 **"왜 8000 인가"** 절 끝에 있다.

### 일곱 개 재측정 — 파지 포함 16D 기록을 먼저 확보했다

재측정에는 파지가 필요한 항목이 있는데(쥔 물체 문턱), **기준선용 16D 기록(120 제어 스텝)에는
파지가 없었다** — 그리퍼가 8 초 동안 계속 1.0(열림)이었다. 그래서 400 제어 스텝(정책 호출 50)
으로 다시 떴고, 거기서 왼 그리퍼가 **호출 36 에서 1.0 → 0.44 로 닫혀 45 까지 유지**된다.
`outputs/live_test/20260924_long16d/run_0000/`.

대상 일곱 개와 진행은 새 로그의 **"물려받은 수치의 모델 출처 — 재측정 대기"** 표에서 관리한다.

---

## 배선 결함 셋을 닫고 판정 쪽으로 내려왔다 (2026-09-25)

R 재측정이 막힌 자리를 세 번 파 보니 **세 번 다 배선이었다.**

| | 무엇이라고 물었나 | 실제 원인 |
|---|---|---|
| T1-a | 14D 의 attention 셀 `L8H2` 가 16D 에서도 성립하나 | **셀 축에 답이 없었다.** 1296 조합 전부 0. attention 맵을 카메라 한 대에만 붙인 배선 |
| X1 | 고정 구 42 개는 너무 많은가 | **분할은 옳다.** 14D 의 27 은 구 105 개 모델이라 비교 대상이 아니었다 |
| X2 | 왜 `cam_high` 에서 목표가 안 잡히나 | **teleop keyframe 이 자기 안에서 모순.** `head_1` 이 0.7 이어야 하는데 기록이 0 으로 떠나 카메라가 벽을 봤다 |

**공통점: 수치가 이상할 때 알고리즘부터 의심했고, 세 번 다 앞단의 연결이었다.**
그림 한 장이면 즉시 보였을 것이 둘(X2 · T1-a)이다 — 규칙 A 가 생긴 이유와 같은 자리다.

### 지금 열려 있는 것 — X3

X2 를 고치고 재촬영하니 지각은 섰다 (`zed_left` 가 사과를 보는 프레임 0/15 → **15/15**).
그러자 **판정 쪽이 갈라졌다**: `clearance_after` 는 전 프레임 양수인데 `status` 는
`violated` 를 반복한다. 계획은 `handoff/X3.task.md`.

**이것이 R4·R5 앞에 선다.** 회귀 기준선의 `feasible` 개수가 무엇을 세는 숫자인지 모르는 채로
backend 를 견주면, 견준 결과도 같은 만큼만 읽힌다.

### 규약 추가 — 로그에 숫자를 적기 전에 raw 를 먼저 남긴다
...
### 사용자 판정 — 실시간성은 T5·T6 으로 미룬다 (2026-09-25)

> *"지금은 실시간성을 고려하지 마라. 맨 나중에 정상 작동되고, 뒤에 가서 실시간성을 잡아먹는
> 부분을 개선하거나 수정하면 된다."*

`pointcloud.max_points` 를 60000 → 200000 으로 올렸다. 회귀 기준선은 한 자리도 안 움직이고
(기준선 기록은 상한에 애초에 안 걸린다 — 점 수 53625~91268), 수정 후 기록은 `feasible` 15/15 다.

**이 판정이 순서를 바꾼다.** 실시간성(청크 예산 533 ms 대비 현재 4.3 배)은 **별도 게이트로
남기되, 앞 게이트의 통과 조건에서 뺀다.** T5(shadow 루프)에서 실제 예산을 잴 때 함께 연다.
그때 줄일 후보는 이미 나와 있다 — `max_points` · OSMesa 렌더링 · ESDF 격자 크기.

**남은 열린 판정**: `DEGRADED`(점군이 상한에 걸려 voxel 이 커진 상태)를 인증 실패로 볼 것인가.
상한을 올려 지금은 안 걸릴 뿐이고, 코드(`types.py:169` · `sqp.py:287`)는 그대로다.

---

## T1 — 연속 프레임 AG3S 완료 (2026-09-25)

**판정 완료. 항목 2·3·5·6·7·8 통과.**

held-out 3 에피소드(사과·바나나·배)에서 실측 정책 attention 과 지각 파이프라인을 검증했다.
각 에피소드 15 프레임, 총 45 프레임. 머리 각도 오류를 고친 뒤 새로 촬영했다(`head_1 = +0.7001`).

### 항목 1 — 점수 문턱이 기능하지 않는다 (에피소드 간 보정 필요)

target_score 와 오차의 상관은 **에피소드 내에서 강하다** (r = −0.949 / −0.985 / −0.944).
그러나 **같은 점수 대역에서 에피소드 간 오차가 10 배 이상 흩어진다:**

- 점수 0.7~0.8: apple 31.9 mm · banana 5.3 mm
- 점수 0.2~0.3: apple 81.9 · banana 320.2 · pear 81.9 mm

**현재 threshold = 0.25** (`config.py:91`). 45 프레임 중:
- **점수 > 0.5 이면서 오차 > 50 mm**: 0 개 — 점수가 높으면 안전하다
- 거른 6 프레임의 "있었다면" 오차: 317.9~327.7 mm (평균 323.4)

**결론**: 점수는 절대 기준이 아니라 **상대 기준**이다. 에피소드마다 보정이 필요하다.

### 항목 4 — self-filter 누수가 두 종류다, 수정 완료 + 판정 완료 둘 (2026-09-25)

카메라 거리값으로 검토하니 **누수의 성질이 갈린다:**

| 누수 출처 | 위치 | 거리(mm) | 점유비율 | 영향 |
|---|---|---|---|---|
| **끝단 구** (EE_BODY_L/R) | 특정 카메라만 (100%) | +104~144 | 0.0 | **무해** — 장애물 아님 |
| **베이스** (base) | 특정 카메라만 (100%) | −8.2 | 0.76~0.99 | **해로움** — 장애물됨 |

**짧은 음수(파고든 쪽)가 해로운 누수고, 긴 양수(먼 쪽)는 무해하다.**
`self_filter_inflation` 을 **0.02 → 0.05 m(50 mm) 로 올렸다.** 결과: `base` 누수 5,758 →
0 px. `EE_BODY_L/R` 은 픽셀 단위로 그대로 — 구가 아예 없어 팽창이 안 닿는다. 그래서
`UNCOVERED_LINKS`(gap-filling capsule 목록)에 `ee_left`·`ee_right` 를 추가했다 — 두 파일이
같은 부품을 다르게 부르는 함정(MJCF `EE_BODY_L` ↔ URDF `ee_left`)이 있어 번역표
`MJCF_BODY_ALIASES` 를 두었다. 자기 필터 구 194 → 218.

부작용: `has_target` 15/15·9/15·15/15 → 14/15·8/15·12/15, 회귀 기준선 시작 15/15 → 14/15 ·
`feasible` 13 → 14 · `violated` 2 → 1.

**판정 완료(2026-09-25)**: (a) `gap_filling_capsules` 의 `except KeyError: continue` (이름이
틀리면 조용히 캡슐 0 개) → **소리를 내게 만든다** — 코드는 이미 그렇게 되어 있다
(`_warn_no_capsules` 가 삼키되 어느 spelling 을 시도했는지까지 `_LOG.warning` 으로 낸다).
(b) 손바닥(`ee_left`/`ee_right`)을 접촉 허용(`DEFAULT_CONTACT_LINKS` 에는 이미 있다)뿐 아니라
**제약** 모델(`rby1_transport.py:454` 는 `ee_finger_` 만 본다)에도 넣을 것인가 → **넣지 않는다,
기존 동작 유지.** 어긋남은 알려진 상태로 남기고, 전환 신호는 [`AG3S_T0T6_LOG.md`](AG3S_T0T6_LOG.md)
T2 절의 **"되돌아올 지점"**. 상세는 같은 문서 T1 **"로봇 마스크 수정"** 절.

### 다음 — 항목 1 의 수정 대기, T2 착수

1. **항목 1**: target_score 의 에피소드별 보정안 필요 — T2 에서 다룬다 (pick-place 상태 전이와 함께)
2. **항목 4**: 수정 완료, 위 판정 둘도 완료(위)

**T2 자산을 확보했다(ep1807, 2026-09-25)** — 처음 뜬 ep1800 은 대상을 들었다가 제자리에
내려놓아 운반이 없었고, ep1807 은 실제로 바구니까지 옮겨져(339 → 71 mm) 과제가 물리적으로
완결된다.

## T2 — nine events 측정 완료, 미측정 셋은 다음 STEP 으로 (2026-09-25)

`ep1807`(2 라운드, 운반 완결) 에서 아홉 사건 중 일곱의 frame 이 확정됐다. **`destination_attention_locked`
와 `task_state_reset` 은 `None`** — 목적지 attention 은 한 번도 `confirm_frames=3` 연속으로
확정되지 않았는데도 `placement`·`detached` 는 frame 32 에서 성공했다(`ep1800` 에서 반쪽이던
뒤 절반이 닫혔다 — `detach` 가 `placed_now` 성공 경로로 울렸고 open-streak fallback 이 아니다).
`detach` 후에도 `latch.phase`(`'placed'`)·`manipulated`(`'obj0'`)가 frame 74 까지 유지돼 T2
합격 조건 *"detach 후 이전 상태가 제거된다"* 는 불합격이지만, 이 기록엔 2 차 파지 시도 자체가
없어 이것이 다음 과제를 실제로 막는지는 증명되지 않는다. 결함 3(`runner_up_score` 키 부재)은
재현되고 결함 2(식별 불일치)는 재현되지 않았다 — 이번 라운드는 대상 참값이 애매하지 않았다.

**사용자 판정 넷(2026-09-25)**: T2 는 미측정 셋(잔상 지속 프레임 수 등)을 채운 뒤 판정한다
(`T2-b` 로 A2 에게) · `runner_up_score` 키 부재는 지금 고친다(`T2-fix` 로 A1 에게) ·
`gap_filling_capsules` 의 조용한 삼킴은 소리를 내게 만든다(코드는 이미 그렇다) · 손바닥을
constraint model 에 넣는 것은 기존 동작을 유지한다. 상세와 수치는
[`AG3S_T0T6_LOG.md`](AG3S_T0T6_LOG.md) 의 **T2** 절.

**T0~T3 중 회귀는 이미 확보됐고** (기준선 기록 1 개 · 수정 후 기록 1 개), R 도 대기 목록에서
벗어난다.

## T2-b — 미측정 둘을 채웠다, **T2 게이트를 부분 통과로 닫는다** (2026-09-25)

`T2` 의 `not_measured` 1·3 번을 `ep1807` 에서 쟀다. 오염 없음을 먼저 확인했다 — 이번 run 은
A1 의 `runner_up_score` 키 추가 **이후** 코드였지만 latch 에는 `runner_up=0.0` 을 고정해
먹였고, 그렇게 나온 75 프레임 latch 계열이 `T2` 의 raw 와 **0 프레임 불일치**였다.

**측정 1(held object 편입)**: attach/detach 로 로봇 기하 편입·제거는 frame 19/32 로 정확하지만,
**attach 가 울린 그 프레임(19)에는 field 쪽 carve 가 아직 안 돌아** 쥔 물체가 로봇 기하와
obstacle field 양쪽에 동시에 존재한다(double counting 1 프레임, frame 20 부터 carve 가 돈다).
candidate 층에는 흔적이 없다. **원인은 코드 읽기로 추정**했을 뿐 측정하지 않았다 —
`safe_policy.py` 가 field 를 만든 뒤에 attach 를 부른다는 순서 문제로 보인다.

**측정 2(TSDF residual)**: decay 를 끈 상태에서 apple 이 떠난 자리의 위쪽 반구 점유가
**26/54 프레임**(frame 21~74 기준) 남고 frame 47 에 영구히 0 이 된다. 정적 대조군(pear)의
위쪽 반구 점유는 75 프레임 내내 0 이 되지 않아 **"원래 있는 것"과 "잔상"이 필드 값만으로는
안 갈린다.**

**사용자 판정(2 차, 2026-09-25)**: T2 게이트는 **부분 통과로 닫고 T3 로 간다**(통과: 조작
대상 ID 유지·held object 기하 편입/제거. 불합격: detach 후 task state reset 없음·destination
attention lock 미확정). carve 1 프레임 지연은 **결함으로 기록하고 T5·T6 에서 본다**(전환
신호: closed loop 에서 파지 순간 가짜 violation 이 실제로 관측되면). TSDF residual 26 프레임은
**T3 의 `max_field_age_sec` 근거**로 T0 의 필드 나이 P50 2637 ms 와 한 짝으로 묶는다. 변경 후
회귀 기준선은 **`T2-c`** 로 A2 에게 나갔다. 상세와 수치는 [`AG3S_T0T6_LOG.md`](AG3S_T0T6_LOG.md)
의 **T2-b** 절.

## T2-c — 변경 후 회귀 기준선: **동일** (2026-09-25)

A1 의 `runner_up_score` 변경 이후 코드(md5 `ee33a055...`)로 회귀 기준선을 다시 쟀다. 단독
실행, 실행 전후 md5 동일. **기준선 셋이 "일치"가 아니라 "동일"이다** — 위반 시작 14/15·
`has_target` 9/15·frame0 `clearance_before` +0.157 mm 이고, **15 프레임 `clearance_before` 가
변경 전 run 과 한 프레임도 다르지 않다**(`n_frames_differing = 0`), `has_target` 플래그 배열도
같다. `feasible`/`violated`(14/1)는 `sqp.time_budget_ms` 가 벽시계 마감이라 흔들리는 값이라
참고로만 적는다.

`runner_up_score` 를 latch 에 **실제로 먹였을 때**도 `ep1807` 75 프레임이 `T2` raw 와 0 프레임
불일치(`attach` 19·`detach` 32 그대로). `confident` 가 갈리는 다섯 프레임([30, 35, 67, 68,
71])은 `T2-b` 의 식 계산과 같았지만, **다섯 전부 `held`/`placed` 구간이다.** `GraspLatch` 는
이미 잠긴 뒤에는 `confident` 를 보지 않는다(`grasp_latch.py:137`). **그래서 이 다섯 프레임에서
`confident` 가 뒤집혀도 latch 는 아무 영향을 안 받고, 이 수정이 실제로 무엇을 하는지는 이
기록으로 시험되지 않았다** — 효과가 나타날 `SEARCHING` 구간에서 갈리는 프레임이 이 기록에
없다. **"기준선이 안 움직였다"를 "이 수정이 안전하다"로 쓰지 않는다.** figure 는 없다(기준선이
안 움직여 지시대로 만들지 않았다). 상세와 수치는 [`AG3S_T0T6_LOG.md`](AG3S_T0T6_LOG.md) 의
**T2-c** 절.
