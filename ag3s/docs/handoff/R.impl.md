...
## 판정 10 — 머리 각도 원인 (**원인 확정 · 수정 확인 · 재촬영은 대기**)

### 원인 — 키프레임이 `qpos` 와 `ctrl` 로 **서로 다른 말을 한다**

`teleop` 키프레임은 머리를 `qpos` 로 `[0.0, 0.7]` 에 두는데, **같은 키프레임의 `ctrl` 은 머리
자리가 0.0** 이다. `mj_resetDataKeyframe` 은 둘 다 그대로 적용하므로 리셋 직후가
`qpos=0.7` · `ctrl=0.0` 인 **모순 상태**가 되고, 머리에 위치 액추에이터가 달려 있어
(`head_0_act`/`head_1_act`) 정착을 도는 순간 `qpos` 가 `ctrl` 로 끌려간다.

내가 `model_transport.xml` 로 직접 잰 것:

```
teleop 키프레임 qpos head = [0.0, 0.7]      key_ctrl 의 머리 자리 = 0.0
리셋 직후      head = [0.0, 0.7    ]
0.5 s 정착 뒤  head = [0.0, 0.02662]
1.5 s 정착 뒤  head = [0.0, 0.00001]
```

`reset_randomized_scene` 의 정착이 `settle_seconds=1.5` 라, 기록에 남은
**`head_1 = 3.94e-05`** 가 정확히 이 값이다. 팔은 이미 리셋 직후 `ctrl` 을 덮어써서
살아남았고(`"…so the position actuators hold the pose instead of dragging it back"`),
**머리만 그 처리를 안 받았다.**

### 수정 — **다른 에이전트가 이미 넣었다. 나는 검증만 했다**

내가 원인을 재는 사이에 `pi05_TO_hybrid/rby1_bringup/pi05_infer.py:259-283` 에
`hold_keyframe_pose(m, d)` 가 들어왔고, 두 리셋 자리 모두에서 불린다 (`:296`, `:950`).
**내 것보다 낫다** — 나는 머리만 잡으려 했는데 이쪽은 **모든 관절 액추에이터**를 자기
키프레임 `qpos` 로 겨눈다. 수집 경로(`transport_scene.py`)가 하는 것과 같아져서 두 경로가
일치한다는 것이 그 docstring 의 근거다. **중복 구현하지 않고 그대로 두었다.**

### 검증 — 머리가 버티고, **사과가 보인다**

```
고치기 전 (hold 없음)          리셋직후 [0. 0.7] -> 1.5s 정착 뒤 [0.  0.00001]
고친 뒤 (hold_keyframe_pose)   리셋직후 [0. 0.7] -> 1.5s 정착 뒤 [0.  0.70007]
```

그리고 **episode 1800 의 첫 프레임을 실제로 렌더해 사과 픽셀을 셌다** (정책 서버 없이,
`reset_randomized_scene` 과 같은 순서로 과일·바구니·팔을 놓고 1.5 s 정착):

| | `head_1` | `zed_left`(=`cam_high`) 사과 | `wrist_cam_l` 사과 |
|---|---|---|---|
| 고치기 전 | `+0.00001` | **0 px** | 0 px |
| **고친 뒤** | `+0.70007` | **548 px** | 0 px |

**`cam_high` 가 사과를 본다.** 축 0(`has_target` 이 틀린 대상을 성공이라 말한다)과 축 E
(grounding 이 `cam_high` 에 의존한다)가 이 기록에서 나타난 이유가 여기였다 — 설계 물음이
아니라 **머리가 정면을 보고 있어서**였다. 축 E 의 설계 질문 자체는 남지만(손목만 대상을 보는
상황이 실기에 있는가), 이 기록은 그 사례가 아니었다.

### 재촬영 — **아직 안 떴다. 지시대로 알리고 기다린다**

지시 4 에 따라 MuJoCo 실행 전에 보고한다. 뜰 준비는 됐고, 뜰 때 지킬 것:

* **새 이름으로** 뜬다 — `run_16d_ep1800_headfix/`. 옛 기록을 덮지 않는다.
* attention npz 도 **새 이름**으로 다시 굽는다 (`attention_16d_ep1800_headfix.npz`).
  옛 npz 는 옛 기록의 프레임에 붙어 있어 섞으면 안 된다.
* 먼저 **`run_16d_ep1800` 하나만.** 긴 기록(50 호출 · 392 제어 스텝)은 그 결과를 보고.
* 필요한 것: 16D 정책 서버(`localhost:8123`). 조율 부탁한다.
