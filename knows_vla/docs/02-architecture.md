...
## 4. 왜 객체를 타원체로 거칠게 표현하는가

논문 §3.1은 EEF와 모든 객체를 **3D 타원체 하나**로 근사한다. 메시도 SDF도 점군도 아니다.
이것은 구현 편의가 아니라 **CBF-QP가 성립하기 위한 요구조건**이다 [DERIVED].

### 타원체여야 하는 이유

**(a) 지지함수가 닫힌 형식이다.** 타원체는 $\max_{y\in\mathcal{E}} n^\top y = n^\top c + \sqrt{n^\top Qn}$
이다. 이 덕분에 Eq. (6)의 barrier가 초등함수로 쓰이고, Eq. (9)–(11)의 gradient 세 개가
**해석적으로** 나온다. 메시나 SDF였다면 매 스텝 거리 질의를 돌리고 gradient를 유한차분으로
근사해야 하며, 20 Hz 예산(§4.3의 11.4 ms QP)에 들어갈 수 없다.

**(b) 볼록해야 초평면 하나로 인증된다.** 분리 초평면 정리는 볼록 집합에만 성립한다. 오목한
형상이면 초평면 하나로 분리를 보장할 수 없어 볼록 분해가 필요하고, 그러면 장애물 하나당
제약이 여러 개로 늘어 QP가 커진다.

**(c) 매끄러워야 gradient가 정의된다.** 박스나 볼록 다면체도 볼록이지만 모서리에서 지지함수가
미분 불가능하다. 타원체는 $Q \succ 0$인 한 어디서나 매끄럽다 — Eq. (10)(11)의 분모
$\sqrt{n^\top Qn}$이 그 대가다.

**(d) 상태가 6+3개뿐이다.** 중심 3 + 형상 6(대칭행렬). 그래서 §3.2가 "형상은 $t=0$에 고정하고
중심만 매 스텝 갱신"할 수 있고, 매 스텝 비용이 centroid 재계산 하나로 끝난다. 논문의 실시간성
주장(9.1 ms)이 여기서 나온다.
...
| 실패 유형 | 결과 | 방향 |
|---|---|---|
| 오목/속이 빈 객체 | 바구니·서랍의 **내부가 메워진다** | 과대 → 과보수 |
| 길쭉하거나 L자 형상 | 빈 공간까지 감싼다 | 과대 → 과보수 |
| 단일 시점 적합 (우리 이식) | 보이는 면만 적합 | **과소 → 위험** |
| $Q$ 고정 가정 | 회전하는 비구형 객체가 어긋남 | 양방향 |

**바구니 사례가 가장 명확하다** [DERIVED]. `libero_object`의 과제는 전부 "X를 집어 바구니에
넣어라"인데, 우리 적합에서 `basket_1`은 9.4 × 12.6 × 13.4 cm의 **속이 찬** 타원체다. 바구니가
장애물 집합에 있으면 **과제가 요구하는 바로 그 동작이 금지된다.**

논문에서 이게 표면화되지 않는 이유는 Eq. (4)가 배치 단계에 목적지를 타깃으로 삼아 제외하기
때문이다. 즉 **타깃 제외는 성능 최적화가 아니라 이 표현이 성립하기 위한 전제**다. gap이
$\delta$ 미만이라 $\tau_t = \varnothing$이 되는 순간 바구니가 장애물로 돌아오고 배치가 불가능해진다.
[11-p2b-results.md](11-p2b-results.md)의 `target=none` 조건이 이 상태를 측정한 것이다.

> 논문 §5의 한계 목록은 "perception error"만 다루고 **표현 자체의 한계**는 언급하지 않는다.
> 지각이 완벽해도 타원체로는 바구니 내부를 표현할 수 없다.
