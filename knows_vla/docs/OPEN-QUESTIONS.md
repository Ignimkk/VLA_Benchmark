...
**해소 방법**: 저자 문의. 불가 시 스윕 — 단, $\delta$는 $\bar A_t$ 정규화 규약(#10)에
스케일 의존적이므로 #10을 먼저 고정해야 한다.
...
## 13. ●● $\delta c_R, \delta\theta$의 크기 제한이 없음

Eq. (12)는 명목값에서의 편차를 벌하지만 **상한이 없다**. 제약이 강하면 정책의 정상 액션
범위를 넘는 delta가 나올 수 있다. 사후 클리핑 여부 미기재.
**위험**: OSC가 추종할 수 없는 명령 → reduced-order 안전 가정(§5) 붕괴.
